<template>
    <h2 class="text-4xl mb-12 row-start-1" :class="{ 'col-start-1': column == 0, 'col-start-2': column == 1 }">
        {{ $t(textKey) }} &nbsp;
        <span v-if="target == 'girl'" class="inline-flex align-middle rounded-full bg-pink-100 px-3 py-0.5 text-base font-medium text-pink-800">Girls</span>
        <span v-if="target == 'boy'" class="inline-flex align-middle rounded-full bg-blue-100 px-3 py-0.5 text-base font-medium text-blue-800">Boys</span>
    </h2>
    <div class="flex flex-col space-y-5 row-start-2" :class="{ 'col-start-1': column == 0, 'col-start-2': column == 1 }">
        <div v-for="idx in ['1','2']" :index="'option-'+idx" class="flex items-center gap-x-2">
            <p class="text-lg w-48">{{ $t(getOption(question.id, idx)) }}</p>
...
const target = computed(() => props.question && props.question.target)
